...
3. **填 Client ID**（二选一，推荐环境变量，别提交到仓库）：
   - Client ID / Secret 已内置在 `app/build.gradle.kts` 默认值里，开箱即用
   - 换成自己的：改这两个 `buildConfigField` 的默认值，或用环境变量覆盖
     `export GITHUB_CLIENT_ID=Iv1.xxx` `export GITHUB_CLIENT_SECRET=xxx`
   - ⚠️ 公开客户端无法真正保护 Secret。仓库公开的话，建议用完就 Revoke 旧的、换新的
